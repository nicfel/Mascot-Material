...
con=file("H3N2_mascot.ape.trees",open="r")
line=readLines(con) 
close(con)
...
pdf("plot.pdf", height=8,width=10)
...
plot(tr,show.tip.label=F, direction="downwards",edge.color=edge_col,root.edge = F,edge.width=1)
tiplabels(pch=21, col=tip_col, adj=c(0.5, 0.5), bg=tip_col, cex=0.5)
nodelabels(pch=21, col=node_col, adj=c(2, 0.5), bg=node_col, cex=0.5)
nodelabels(pch=21, col=node_col, adj=c(-1, 0.5), bg=node_col, cex=0.75)

axisPhylo(side = 2, las=1,root.time = 2002.742466-root_heigth, backward = F)
